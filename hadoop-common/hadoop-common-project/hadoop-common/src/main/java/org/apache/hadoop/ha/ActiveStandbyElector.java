...
public class ActiveStandbyElector implements @Tainted StatCallback, @Tainted StringCallback {
  ...
  public interface ActiveStandbyElectorCallback {
     ...
     */
    void becomeActive(ActiveStandbyElector.@Tainted ActiveStandbyElectorCallback this) throws ServiceFailedException;

    /**
     * This method is called when the app becomes a standby
     */
    void becomeStandby(ActiveStandbyElector.@Tainted ActiveStandbyElectorCallback this);
     ...
     * transient issues.
     */
    void enterNeutralMode(ActiveStandbyElector.@Tainted ActiveStandbyElectorCallback this);
     ...
     */
    void notifyFatalError(ActiveStandbyElector.@Tainted ActiveStandbyElectorCallback this, @Tainted String errorMessage);
     ...
     */
    void fenceOldActive(ActiveStandbyElector.@Tainted ActiveStandbyElectorCallback this, @Tainted byte @Tainted [] oldActiveData);
  }
   ...
   */
  @VisibleForTesting
  protected static final @Tainted String LOCK_FILENAME = "ActiveStandbyElectorLock";
  @VisibleForTesting
  protected static final @Tainted String BREADCRUMB_FILENAME = "ActiveBreadCrumb";

  public static final @Tainted Log LOG = LogFactory.getLog(ActiveStandbyElector.class);

  static @Tainted int NUM_RETRIES = 3;
  private static final @Tainted int SLEEP_AFTER_FAILURE_TO_BECOME_ACTIVE = 1000;

  private static enum ConnectionState {

@Tainted  DISCONNECTED,  @Tainted  CONNECTED,  @Tainted  TERMINATED
  };

  static enum State {

@Tainted  INIT,  @Tainted  ACTIVE,  @Tainted  STANDBY,  @Tainted  NEUTRAL
  };

  private @Tainted State state = State.INIT;
  private @Tainted int createRetryCount = 0;
  private @Tainted int statRetryCount = 0;
  private @Tainted ZooKeeper zkClient;
  private @Tainted WatcherWithClientRef watcher;
  private @Tainted ConnectionState zkConnectionState = ConnectionState.TERMINATED;

  private final @Tainted ActiveStandbyElectorCallback appClient;
  private final @Tainted String zkHostPort;
  private final @Tainted int zkSessionTimeout;
  private final @Tainted List<@Tainted ACL> zkAcl;
  private final @Tainted List<@Tainted ZKAuthInfo> zkAuthInfo;
  private @Tainted byte @Tainted [] appData;
  private final @Tainted String zkLockFilePath;
  private final @Tainted String zkBreadCrumbPath;
  private final @Tainted String znodeWorkingDir;

  private @Tainted Lock sessionReestablishLockForTests = new @Tainted ReentrantLock();
  private @Tainted boolean wantToBeInElection;
   ...
   */
  public @Tainted ActiveStandbyElector(@Tainted String zookeeperHostPorts,
      @Tainted
      int zookeeperSessionTimeout, @Tainted String parentZnodeName, @Tainted List<@Tainted ACL> acl,
      @Tainted
      List<@Tainted ZKAuthInfo> authInfo,
      @Tainted
      ActiveStandbyElectorCallback app) throws IOException,
      HadoopIllegalArgumentException, KeeperException {
    if (app == null || acl == null || parentZnodeName == null
        || zookeeperHostPorts == null || zookeeperSessionTimeout <= 0) {
      throw new @Tainted HadoopIllegalArgumentException("Invalid argument");
    }
   ...
   */
  public synchronized void joinElection(@Tainted ActiveStandbyElector this, @Tainted byte @Tainted [] data)
      throws HadoopIllegalArgumentException {
    
    if (data == null) {
      throw new @Tainted HadoopIllegalArgumentException("data cannot be null");
    }
    ...
    appData = new @Tainted byte @Tainted [data.length];
   ...
   */
  public synchronized @Tainted boolean parentZNodeExists(@Tainted ActiveStandbyElector this)
      throws IOException, InterruptedException {
    Preconditions.checkState(zkClient != null);
    try {
      return zkClient.exists(znodeWorkingDir, false) != null;
    } catch (@Tainted KeeperException e) {
      throw new @Tainted IOException("Couldn't determine existence of znode '" +
          znodeWorkingDir + "'", e);
    }
  }
   ...
   */
  public synchronized void ensureParentZNode(@Tainted ActiveStandbyElector this)
      throws IOException, InterruptedException {
    Preconditions.checkState(!wantToBeInElection,
        "ensureParentZNode() may not be called while in the election");

    @Tainted
    String pathParts @Tainted [] = znodeWorkingDir.split("/");
    ...
    for (@Tainted int i = 1; i < pathParts.length; i++) {
      sb.append("/").append(pathParts[i]);
      @Tainted
      String prefixPath = sb.toString();
      LOG.debug("Ensuring existence of " + prefixPath);
      try {
        createWithRetries(prefixPath, new @Tainted byte @Tainted []{}, zkAcl, CreateMode.PERSISTENT);
      } catch (@Tainted KeeperException e) {
        if (isNodeExists(e.code())) {
          // This is OK - just ensuring existence.
          continue;
        } else {
          throw new @Tainted IOException("Couldn't create " + prefixPath, e);
        }
      }
    }
   ...
   */
  public synchronized void clearParentZNode(@Tainted ActiveStandbyElector this)
      throws IOException, InterruptedException {
    Preconditions.checkState(!wantToBeInElection,
    ...
    try {
      ...
      zkDoWithRetries(new @Tainted ZKAction<@Tainted Void>() {
        @Override
        public @Tainted Void run() throws KeeperException, InterruptedException {
          ZKUtil.deleteRecursive(zkClient, znodeWorkingDir);
          return null;
        }
      });
    } catch (@Tainted KeeperException e) {
      throw new @Tainted IOException("Couldn't clear parent znode " + znodeWorkingDir,
          e);
    }
   ...
   */
  public synchronized void quitElection(@Tainted ActiveStandbyElector this, @Tainted boolean needFence) {
   ...
   */
  public static class ActiveNotFoundException extends @Tainted Exception {
    private static final @Tainted long serialVersionUID = 3505396722342846462L;
  }
   ...
   */
  public synchronized @Tainted byte @Tainted [] getActiveData(@Tainted ActiveStandbyElector this) throws ActiveNotFoundException,
    ...
    try {
      ...
      if (isNodeDoesNotExist(code)) {
        ...
        throw new @Tainted ActiveNotFoundException();
      } else {
        throw e;
      }
    }
   ...
   */
  @Override
  public synchronized void processResult(@Tainted ActiveStandbyElector this, @Tainted int rc, @Tainted String path, @Tainted Object ctx,
      @Tainted
      String name) {
   ...
   * becomeActive().
   */
  private void reJoinElectionAfterFailureToBecomeActive(@Tainted ActiveStandbyElector this) {
   ...
   */
  synchronized void processWatchEvent(@Tainted ActiveStandbyElector this, @Tainted ZooKeeper zk, @Tainted WatchedEvent event) {
    Event.@Tainted EventType eventType = event.getType();
   ...
   */
  protected synchronized @Tainted ZooKeeper getNewZooKeeper(@Tainted ActiveStandbyElector this) throws IOException,
      KeeperException {
    ...
    watcher = new @Tainted WatcherWithClientRef();
    @Tainted
    ZooKeeper zk = new @Tainted ZooKeeper(zkHostPort, zkSessionTimeout, watcher);
    ...
    for (@Tainted ZKAuthInfo auth : zkAuthInfo) {
      zk.addAuthInfo(auth.getScheme(), auth.getAuth());
    }
    return zk;
  }

  private void fatalError(@Tainted ActiveStandbyElector this, @Tainted String errorMessage) {
    LOG.fatal(errorMessage);
    reset();
    appClient.notifyFatalError(errorMessage);
  }

  private void monitorActiveStatus(@Tainted ActiveStandbyElector this) {
    assert wantToBeInElection;
  ...
  private void joinElectionInternal(@Tainted ActiveStandbyElector this) {
  ...
  private void reJoinElection(@Tainted ActiveStandbyElector this, @Tainted int sleepTime) {
   ...
   */
  @VisibleForTesting
  protected void sleepFor(@Tainted ActiveStandbyElector this, @Tainted int sleepMs) {
    if (sleepMs > 0) {
      try {
  ...
  @VisibleForTesting
  void preventSessionReestablishmentForTests(@Tainted ActiveStandbyElector this) {
    sessionReestablishLockForTests.lock();
  }
  
  @VisibleForTesting
  void allowSessionReestablishmentForTests(@Tainted ActiveStandbyElector this) {
    sessionReestablishLockForTests.unlock();
  }
  
  @VisibleForTesting
  synchronized @Tainted long getZKSessionIdForTests(@Tainted ActiveStandbyElector this) {
    if (zkClient != null) {
  ...
  @VisibleForTesting
  synchronized @Tainted State getStateForTests(@Tainted ActiveStandbyElector this) {
    return state;
  }

  private @Tainted boolean reEstablishSession(@Tainted ActiveStandbyElector this) {
    @Tainted
  ...
  private void createConnection(@Tainted ActiveStandbyElector this) throws IOException, KeeperException {
    if (zkClient != null) {
      try {
        zkClient.close();
      } catch (@Tainted InterruptedException e) {
        throw new @Tainted IOException("Interrupted while closing ZK",
            e);
      }
      zkClient = null;
      watcher = null;
  ...
  void terminateConnection(@Tainted ActiveStandbyElector this) {
    if (zkClient == null) {
      return;
    }
  ...
  private void reset(@Tainted ActiveStandbyElector this) {
    state = State.INIT;
    terminateConnection();
  }

  private @Tainted boolean becomeActive(@Tainted ActiveStandbyElector this) {
    assert wantToBeInElection;
    if (state == State.ACTIVE) {
      // already active
      return true;
   ...
   */
  private void writeBreadCrumbNode(@Tainted ActiveStandbyElector this, @Tainted Stat oldBreadcrumbStat)
   ...
   */
  private void tryDeleteOwnBreadCrumbNode(@Tainted ActiveStandbyElector this) {
   ...
   */
  private @Tainted Stat fenceOldActive(@Tainted ActiveStandbyElector this) throws InterruptedException, KeeperException {
    final @Tainted Stat stat = new @Tainted Stat();
    @Tainted
    byte @Tainted [] data;
    LOG.info("Checking for any old active which needs to be fenced...");
    try {
      data = zkDoWithRetries(new @Tainted ZKAction<@Tainted byte @Tainted []>() {
        @Override
        public @Tainted byte @Tainted [] run() throws KeeperException, InterruptedException {
  ...
  private void becomeStandby(@Tainted ActiveStandbyElector this) {
  ...
  private void enterNeutralMode(@Tainted ActiveStandbyElector this) {
  ...
  private void createLockNodeAsync(@Tainted ActiveStandbyElector this) {
    zkClient.create(zkLockFilePath, appData, zkAcl, CreateMode.EPHEMERAL,
        this, zkClient);
  }

  private void monitorLockNodeAsync(@Tainted ActiveStandbyElector this) {
    zkClient.exists(zkLockFilePath, 
        watcher, this,
        zkClient);
  }

  private @Tainted String createWithRetries(@Tainted ActiveStandbyElector this, final @Tainted String path, final @Tainted byte @Tainted [] data,
      final @Tainted List<@Tainted ACL> acl, final @Tainted CreateMode mode)
      throws InterruptedException, KeeperException {
    return zkDoWithRetries(new @Tainted ZKAction<@Tainted String>() {
      @Override
      public @Tainted String run() throws KeeperException, InterruptedException {
        return zkClient.create(path, data, acl, mode);
      }
    });
  }

  private @Tainted byte @Tainted [] getDataWithRetries(@Tainted ActiveStandbyElector this, final @Tainted String path, final @Tainted boolean watch,
      final @Tainted Stat stat) throws InterruptedException, KeeperException {
    return zkDoWithRetries(new @Tainted ZKAction<@Tainted byte @Tainted []>() {
      @Override
      public @Tainted byte @Tainted [] run() throws KeeperException, InterruptedException {
  ...
  private @Tainted Stat setDataWithRetries(@Tainted ActiveStandbyElector this, final @Tainted String path, final @Tainted byte @Tainted [] data,
      final @Tainted int version) throws InterruptedException, KeeperException {
    return zkDoWithRetries(new @Tainted ZKAction<@Tainted Stat>() {
      @Override
      public @Tainted Stat run() throws KeeperException, InterruptedException {
        return zkClient.setData(path, data, version);
      }
    });
  }
  
  private void deleteWithRetries(@Tainted ActiveStandbyElector this, final @Tainted String path, final @Tainted int version)
      throws KeeperException, InterruptedException {
    zkDoWithRetries(new @Tainted ZKAction<@Tainted Void>() {
      @Override
      public @Tainted Void run() throws KeeperException, InterruptedException {
  ...
  private static <@Tainted T extends java.lang.@Tainted Object> @Tainted T zkDoWithRetries(@Tainted ZKAction<@Tainted T> action)
      throws KeeperException, InterruptedException {
    @Tainted
    int retry = 0;
    while (true) {
  ...
  private interface ZKAction<@Tainted T extends java.lang.@Tainted Object> {
    @Tainted
    T run(ActiveStandbyElector.@Tainted ZKAction<T> this) throws KeeperException, InterruptedException; 
  }
   ...
   */
  private synchronized @Tainted boolean isStaleClient(@Tainted ActiveStandbyElector this, @Tainted Object ctx) {
    Preconditions.checkNotNull(ctx);
    if (zkClient != (@Tainted ZooKeeper)ctx) {
      LOG.warn("Ignoring stale result from old client with sessionId " +
          String.format("0x%08x", ((@Tainted ZooKeeper)ctx).getSessionId()));
      return true;
    }
    return false;
  }
   ...
   * events.
   */
  private final class WatcherWithClientRef implements @Tainted Watcher {
    private @Tainted ZooKeeper zk;
     ...
     */
    private @Tainted CountDownLatch hasReceivedEvent = new @Tainted CountDownLatch(1);

    /**
     * Latch used to wait until the reference to ZooKeeper is set.
     */
    private @Tainted CountDownLatch hasSetZooKeeper = new @Tainted CountDownLatch(1);
     ...
     */
    private void waitForZKConnectionEvent(@Tainted ActiveStandbyElector.WatcherWithClientRef this, @Tainted int connectionTimeoutMs)
    ...
    private void setZooKeeperRef(@Tainted ActiveStandbyElector.WatcherWithClientRef this, @Tainted ZooKeeper zk) {
    ...
    @Override
    public void process(@Tainted ActiveStandbyElector.WatcherWithClientRef this, @Tainted WatchedEvent event) {
  ...
  private static @Tainted boolean isSuccess(@Tainted Code code) {
    return (code == Code.OK);
  }

  private static @Tainted boolean isNodeExists(@Tainted Code code) {
    return (code == Code.NODEEXISTS);
  }

  private static @Tainted boolean isNodeDoesNotExist(@Tainted Code code) {
    return (code == Code.NONODE);
  }
  
  private static @Tainted boolean isSessionExpired(@Tainted Code code) {
    return (code == Code.SESSIONEXPIRED);
  }

  private static @Tainted boolean shouldRetry(@Tainted Code code) {
    switch (code) {
    case CONNECTIONLOSS:
  ...
  @Override
  public @Tainted String toString(@Tainted ActiveStandbyElector this) {
