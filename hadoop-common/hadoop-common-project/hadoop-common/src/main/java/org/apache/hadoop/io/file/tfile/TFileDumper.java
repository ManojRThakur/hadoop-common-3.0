 ...
 */
class TFileDumper {
  static final @Tainted Log LOG = LogFactory.getLog(TFileDumper.class);

  private @Tainted TFileDumper() {
    // namespace object not constructable.
  }

  private enum Align {

@Tainted  LEFT,  @Tainted  CENTER,  @Tainted  RIGHT,  @Tainted  ZERO_PADDED;
    static @Tainted String format(@Tainted String s, @Tainted int width, @Tainted Align align) {
      ...
      throw new @Tainted IllegalArgumentException("Unsupported alignment");
    }

    static @Tainted String format(@Tainted long l, @Tainted int width, @Tainted Align align) {
    ...
    static @Tainted int calculateWidth(@Tainted String caption, @Tainted long max) {
   ...
   * @throws IOException
   */
  static public void dumpInfo(@Tainted String file, @Tainted PrintStream out, @Tainted Configuration conf)
      throws IOException {
    final @Tainted int maxKeySampleLen = 16;
    @Tainted
    Path path = new @Tainted Path(file);
    @Tainted
    ...
    TFile.@Tainted Reader reader = new TFile.@Tainted Reader(fsdis, length, conf);
    try {
      @Tainted
      LinkedHashMap<@Tainted String, @Tainted String> properties =
          new @Tainted LinkedHashMap<@Tainted String, @Tainted String>();
      @Tainted
      ...
      if (blockCnt > 0) {
        for (@Tainted int i = 0; i < blockCnt; ++i) {
          @Tainted
          BlockRegion region =
        ...
        if (!reader.readerBCF.getDefaultCompressionName().equals("none")) {
          ...
          properties.put("Data Block Compression Ratio", String.format(
              "1:%.1f", (@Tainted double) dataSizeUncompressed / dataSize));
        }
      }
      ...
      if (metaBlkCnt > 0) {
        @Tainted
        Collection<@Tainted MetaIndexEntry> metaBlks =
            reader.readerBCF.metaIndex.index.values();
        @Tainted
        boolean calculateCompression = false;
        for (@Tainted Iterator<@Tainted MetaIndexEntry> it = metaBlks.iterator(); it.hasNext();) {
          @Tainted
        ...
        if (calculateCompression) {
          ...
          properties.put("Meta Block Compression Ratio", String.format(
              "1:%.1f", (@Tainted double) metaSizeUncompressed / metaSize));
        }
      }
      properties.put("Meta-Data Size Ratio", String.format("1:%.1f",
          (@Tainted double) dataSize / metaSize));
      @Tainted
      ...
      int maxKeyLength = 0;
      @Tainted
      Set<Map.@Tainted Entry<@Tainted String, @Tainted String>> entrySet = properties.entrySet();
      for (@Tainted Iterator<Map.@Tainted Entry<@Tainted String, @Tainted String>> it = entrySet.iterator(); it
          .hasNext();) {
        Map.@Tainted Entry<@Tainted String, @Tainted String> e = it.next();
      ...
      if (blockCnt > 0) {
        ...
        for (@Tainted int i = 0; i < blockCnt; ++i) {
          ...
          if (!asAscii) {
            ...
            out.print(new @Tainted String(key, 0, sampleLen));
          }
      ...
      if (metaBlkCnt > 0) {
        ...
        int maxNameLen = 0;
        @Tainted
        Set<Map.@Tainted Entry<@Tainted String, @Tainted MetaIndexEntry>> metaBlkEntrySet =
            reader.readerBCF.metaIndex.index.entrySet();
        for (@Tainted Iterator<Map.@Tainted Entry<@Tainted String, @Tainted MetaIndexEntry>> it =
            metaBlkEntrySet.iterator(); it.hasNext();) {
          Map.@Tainted Entry<@Tainted String, @Tainted MetaIndexEntry> e = it.next();
