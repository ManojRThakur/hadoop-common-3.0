...
public class SaslOutputStream extends @Tainted OutputStream {

  private final @Tainted OutputStream outStream;
  ...
  private final @Tainted byte @Tainted [] ibuffer = new @Tainted byte @Tainted [1];
   ...
   */
  public @Tainted SaslOutputStream(@Tainted OutputStream outStream, @Tainted SaslServer saslServer) {
    ...
    if (useWrap) {
      this.outStream = new @Tainted BufferedOutputStream(outStream, 64*1024);
    } else {
   ...
   */
  public @Tainted SaslOutputStream(@Tainted OutputStream outStream, @Tainted SaslClient saslClient) {
   ...
   */
  private void disposeSasl(@Tainted SaslOutputStream this) throws SaslException {
    if (saslClient != null) {
      saslClient.dispose();
    }
    if (saslServer != null) {
   ...
   */
  @Override
  public void write(@Tainted SaslOutputStream this, @Tainted int b) throws IOException {
    if (!useWrap) {
      outStream.write(b);
      return;
    }
    ibuffer[0] = (@Tainted byte) b;
    write(ibuffer, 0, 1);
  }
   ...
   */
  @Override
  public void write(@Tainted SaslOutputStream this, @Tainted byte @Tainted [] b) throws IOException {
    write(b, 0, b.length);
  }
   ...
   */
  @Override
  public void write(@Tainted SaslOutputStream this, @Tainted byte @Tainted [] inBuf, @Tainted int off, @Tainted int len) throws IOException {
    if (!useWrap) {
    ...
    if (saslToken != null) {
      @Tainted
      ByteArrayOutputStream byteOut = new @Tainted ByteArrayOutputStream();
      @Tainted
      DataOutputStream dout = new @Tainted DataOutputStream(byteOut);
   ...
   */
  @Override
  public void flush(@Tainted SaslOutputStream this) throws IOException {
    outStream.flush();
  }
   ...
   */
  @Override
  public void close(@Tainted SaslOutputStream this) throws IOException {
    disposeSasl();
    outStream.close();
  }
}