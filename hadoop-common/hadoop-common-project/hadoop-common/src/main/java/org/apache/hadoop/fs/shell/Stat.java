...
class Stat extends @Tainted FsCommand {
  public static void registerCommands(@Tainted CommandFactory factory) {
    factory.addClass(Stat.class, "-stat");
  }

  public static final @Tainted String NAME = "stat";
  public static final @Tainted String USAGE = "[format] <path> ...";
  ...
  protected static final @Tainted SimpleDateFormat timeFmt;
  static {
    timeFmt = new @Tainted SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    timeFmt.setTimeZone(TimeZone.getTimeZone("UTC"));
  }

  // default format string
  protected @Tainted String format = "%y";

  @Override
  protected void processOptions(@Tainted Stat this, @Tainted LinkedList<@Tainted String> args) throws IOException {
    @Tainted
    CommandFormat cf = new @Tainted CommandFormat(1, Integer.MAX_VALUE, "R");
    cf.parse(args);
  ...
  @Override
  protected void processPath(@Tainted Stat this, @Tainted PathData item) throws IOException {
    @Tainted
    ...
    @Tainted
    char @Tainted [] fmt = format.toCharArray();
    for (@Tainted int i = 0; i < fmt.length; ++i) {
      if (fmt[i] != '%') {
        buf.append(fmt[i]);
      } else {
        ...
        switch (fmt[++i]) {
          ...
          case 'u':
            ...
            break;
          case 'y':
            buf.append(timeFmt.format(new @Tainted Date(stat.getModificationTime())));
            break;
          case 'Y':
