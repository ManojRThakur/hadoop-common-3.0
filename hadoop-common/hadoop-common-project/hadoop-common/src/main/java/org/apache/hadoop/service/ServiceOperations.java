...
public final class ServiceOperations {
  ...
  private @Tainted ServiceOperations() {
  }
   ...
   */
  public static void stop(@Tainted Service service) {
    if (service != null) {
      service.stop();
    }
  }
   ...
   */
  public static @Tainted Exception stopQuietly(@Tainted Service service) {
   ...
   */
  public static @Tainted Exception stopQuietly(@Tainted Log log, @Tainted Service service) {
    try {
      stop(service);
  ...
  public static class ServiceListeners {
     ...
     */
    private final @Tainted List<@Tainted ServiceStateChangeListener> listeners =
      new @Tainted ArrayList<@Tainted ServiceStateChangeListener>();
     ...
     * @param l listener
     */
    public synchronized void add(ServiceOperations.@Tainted ServiceListeners this, @Tainted ServiceStateChangeListener l) {
     ...
     */
    public synchronized @Tainted boolean remove(ServiceOperations.@Tainted ServiceListeners this, @Tainted ServiceStateChangeListener l) {
     ...
     */
    public synchronized void reset(ServiceOperations.@Tainted ServiceListeners this) {
      listeners.clear();
    }
     ...
     */
    public void notifyListeners(ServiceOperations.@Tainted ServiceListeners this, @Tainted Service service) {
      //take a very fast snapshot of the callback list
      //very much like CopyOnWriteArrayList, only more minimal
      @Tainted
      ServiceStateChangeListener @Tainted [] callbacks;
      synchronized (this) {
        callbacks = listeners.toArray(new @Tainted ServiceStateChangeListener @Tainted [listeners.size()]);
      }
      ...
      for (@Tainted ServiceStateChangeListener l : callbacks) {
