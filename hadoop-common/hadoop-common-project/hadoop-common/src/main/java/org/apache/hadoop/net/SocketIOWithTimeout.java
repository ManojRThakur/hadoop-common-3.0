...
abstract class SocketIOWithTimeout {
  ...
  static final @Tainted Log LOG = LogFactory.getLog(SocketIOWithTimeout.class);    
  
  private @Tainted SelectableChannel channel;
  private @Tainted long timeout;
  private @Tainted boolean closed = false;
  
  private static @Tainted SelectorPool selector = new @Tainted SelectorPool();
   ...
   */
  @Tainted
  SocketIOWithTimeout(@Tainted SelectableChannel channel, @Tainted long timeout) 
                                                 throws IOException {
  ...
  void close(@Tainted SocketIOWithTimeout this) {
    closed = true;
  }

  @Tainted
  boolean isOpen(@Tainted SocketIOWithTimeout this) {
    return !closed && channel.isOpen();
  }

  @Tainted
  SelectableChannel getChannel(@Tainted SocketIOWithTimeout this) {
    return channel;
  }
   ...
   * reasons.
   */
  static void checkChannelValidity(@Tainted Object channel) throws IOException {
    if (channel == null) {
       ...
       */
      throw new @Tainted IOException("Channel is null. Check " +
                            "how the channel or socket is created.");
    }
    
    if (!(channel instanceof @Tainted SelectableChannel)) {
      throw new @Tainted IOException("Channel should be a SelectableChannel");
    }    
  }
   ...
   * @throws IOException
   */
  abstract @Tainted int performIO(@Tainted SocketIOWithTimeout this, @Tainted ByteBuffer buf) throws IOException;  
   ...
   */
  @Tainted
  int doIO(@Tainted SocketIOWithTimeout this, @Tainted ByteBuffer buf, @Tainted int ops) throws IOException {
     ...
     */
    if (!buf.hasRemaining()) {
      throw new @Tainted IllegalArgumentException("Buffer has no data left.");
    ...
    while (buf.hasRemaining()) {
      ...
      } catch (@Tainted IOException e) { //unexpected IOException.
        closed = true;
        throw e;
      } 

      if (count == 0) {
        throw new @Tainted SocketTimeoutException(timeoutExceptionString(channel,
                                                                timeout, ops));
      }
   ...
   * @throws IOException
   */
  static void connect(@Tainted SocketChannel channel, 
                      @Tainted
                      SocketAddress endpoint, @Tainted int timeout) throws IOException {
    ...
    try { 
      ...
      while (true) {
        ...
        @Tainted
        int ret = selector.select((@Tainted SelectableChannel)channel, 
        ...
        if (ret == 0 ||
          ...
          throw new @Tainted SocketTimeoutException(
   ...
   */
  void waitForIO(@Tainted SocketIOWithTimeout this, @Tainted int ops) throws IOException {
    
    if (selector.select(channel, ops, timeout) == 0) {
      throw new @Tainted SocketTimeoutException(timeoutExceptionString(channel, timeout,
                                                              ops)); 
    }
  }

  public void setTimeout(@Tainted SocketIOWithTimeout this, @Tainted long timeoutMs) {
    this.timeout = timeoutMs;
  }
    
  private static @Tainted String timeoutExceptionString(@Tainted SelectableChannel channel,
                                               @Tainted
                                               long timeout, @Tainted int ops) {
  ...
  private static class SelectorPool {
    ...
    private static class SelectorInfo {
      ...
      @Tainted
      LinkedList<@Tainted SelectorInfo> queue; 
      
      void close(SocketIOWithTimeout.SelectorPool.@Tainted SelectorInfo this) {
        if (selector != null) {
          try {
            selector.close();
          } catch (@Tainted IOException e) {
    ...
    private static class ProviderInfo {
      ...
      @Tainted
      LinkedList<@Tainted SelectorInfo> queue; // lifo
      @Tainted
      ProviderInfo next;
    }
    
    private static final @Tainted long IDLE_TIMEOUT = 10 * 1000; // 10 seconds.
    
    private @Tainted ProviderInfo providerList = null;
     ...
     */
    @Tainted
    int select(SocketIOWithTimeout.@Tainted SelectorPool this, @Tainted SelectableChannel channel, @Tainted int ops, @Tainted long timeout) 
                                                   throws IOException {
      ...
      try {
        while (true) {
          ...
          if (Thread.currentThread().isInterrupted()) {
            throw new @Tainted InterruptedIOException("Interruped while waiting for " +
     ...
     * @throws IOException
     */
    private synchronized @Tainted SelectorInfo get(SocketIOWithTimeout.@Tainted SelectorPool this, @Tainted SelectableChannel channel) 
                                                         throws IOException {
      @Tainted
      ...
      if (pList == null) {
        ...
        pList = new @Tainted ProviderInfo();
        pList.provider = provider;
        pList.queue = new @Tainted LinkedList<@Tainted SelectorInfo>();
        pList.next = providerList;
        providerList = pList;
      }
      
      @Tainted
      LinkedList<@Tainted SelectorInfo> queue = pList.queue;
      
      if (queue.isEmpty()) {
        @Tainted
        Selector selector = provider.openSelector();
        selInfo = new @Tainted SelectorInfo();
     ...
     * @param info
     */
    private synchronized void release(SocketIOWithTimeout.@Tainted SelectorPool this, @Tainted SelectorInfo info) {
      @Tainted
      long now = Time.now();
     ...
     * the timeout.
     */
    private void trimIdleSelectors(SocketIOWithTimeout.@Tainted SelectorPool this, @Tainted long now) {
      @Tainted
      long cutoff = now - IDLE_TIMEOUT;
      
      for(@Tainted ProviderInfo pList=providerList; pList != null; pList=pList.next) {
        if (pList.queue.isEmpty()) {
          continue;
        }
        for(@Tainted Iterator<@Tainted SelectorInfo> it = pList.queue.iterator(); it.hasNext();) {
          @Tainted
