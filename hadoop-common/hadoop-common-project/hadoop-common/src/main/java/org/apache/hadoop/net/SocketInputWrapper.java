...
public class SocketInputWrapper extends @Tainted FilterInputStream {
  private final @Tainted Socket socket;
  private final @Tainted boolean hasChannel;

  @Tainted
  SocketInputWrapper(@Tainted Socket s, @Tainted InputStream is) {
    super(is);
    this.socket = s;
    this.hasChannel = s.getChannel() != null;
    if (hasChannel) {
      Preconditions.checkArgument(is instanceof @Tainted SocketInputStream,
   ...
   */
  public void setTimeout(@Tainted SocketInputWrapper this, @Tainted long timeoutMs) throws SocketException {
    if (hasChannel) {
      ((@Tainted SocketInputStream)in).setTimeout(timeoutMs);
    } else {
      socket.setSoTimeout((@Tainted int)timeoutMs);
    }
  }
   ...
   */
  public @Tainted ReadableByteChannel getReadableByteChannel(@Tainted SocketInputWrapper this) {
    Preconditions.checkState(hasChannel,
        "Socket %s does not have a channel",
        this.socket);
    return (@Tainted SocketInputStream)in;
  }
}