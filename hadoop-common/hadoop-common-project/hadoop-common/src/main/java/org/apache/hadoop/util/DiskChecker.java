...
public class DiskChecker {
  ...
  private static final @Tainted long SHELL_TIMEOUT = 10 * 1000;

  public static class DiskErrorException extends @Tainted IOException {
    public @Tainted DiskErrorException(@Tainted String msg) {
      super(msg);
    }

    public @Tainted DiskErrorException(@Tainted String msg, @Tainted Throwable cause) {
      super(msg, cause);
    }
  }
    
  public static class DiskOutOfSpaceException extends @Tainted IOException {
    public @Tainted DiskOutOfSpaceException(@Tainted String msg) {
      super(msg);
    }
  }
   ...
   */
  public static @Tainted boolean mkdirsWithExistsCheck(@Tainted File dir) {
    ...
    return (parent != null) && 
           (mkdirsWithExistsCheck(new @Tainted File(parent)) &&
   ...
   */
  public static void checkDir(@Tainted File dir) throws DiskErrorException {
    if (!mkdirsWithExistsCheck(dir)) {
      throw new @Tainted DiskErrorException("Can not create directory: "
                                   + dir.toString());
    }
    checkDirAccess(dir);
  }
  ...
  public static void mkdirsWithExistsAndPermissionCheck(
      @Tainted
      LocalFileSystem localFS, @Tainted Path dir, @Tainted FsPermission expected)
      throws IOException {
    @Tainted
   ...
   * @throws IOException
   */
  public static void checkDir(@Tainted LocalFileSystem localFS, @Tainted Path dir,
                              @Tainted
   ...
   */
  private static void checkDirAccess(@Tainted File dir) throws DiskErrorException {
    if (!dir.isDirectory()) {
      throw new @Tainted DiskErrorException("Not a directory: "
                                   + dir.toString());
    }
   ...
   *   executable
   */
  private static void checkAccessByFileMethods(@Tainted File dir)
      throws DiskErrorException {
    if (!FileUtil.canRead(dir)) {
      throw new @Tainted DiskErrorException("Directory is not readable: "
                                   + dir.toString());
    }

    if (!FileUtil.canWrite(dir)) {
      throw new @Tainted DiskErrorException("Directory is not writable: "
                                   + dir.toString());
    }

    if (!FileUtil.canExecute(dir)) {
      throw new @Tainted DiskErrorException("Directory is not executable: "
                                   + dir.toString());
    }
  }
}
