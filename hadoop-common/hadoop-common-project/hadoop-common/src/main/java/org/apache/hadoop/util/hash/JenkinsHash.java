...
public class JenkinsHash extends @Tainted Hash {
  private static @Tainted long INT_MASK  = 0x00000000ffffffffL;
  private static @Tainted long BYTE_MASK = 0x00000000000000ffL;
  
  private static @Tainted JenkinsHash _instance = new @Tainted JenkinsHash();
  
  public static @Tainted Hash getInstance() {
    return _instance;
  }

  private static @Tainted long rot(@Tainted long val, @Tainted int pos) {
    return ((Integer.rotateLeft(
        (@Tainted int)(val & INT_MASK), pos)) & INT_MASK);
  }
  ...
  */
  ...
  public @Tainted int hash(@Tainted JenkinsHash this, @Tainted byte @Tainted [] key, @Tainted int nbytes, @Tainted int initval) {
    @Tainted
    int length = nbytes;
   ...
   * @throws IOException
   */
  public static void main(@Tainted String @Tainted [] args) throws IOException {
    if (args.length != 1) {
      System.err.println("Usage: JenkinsHash filename");
      System.exit(-1);
    }
    @Tainted
    FileInputStream in = new @Tainted FileInputStream(args[0]);
    @Tainted
    byte @Tainted [] bytes = new @Tainted byte @Tainted [512];
    @Tainted
    int value = 0;
    @Tainted
    JenkinsHash hash = new @Tainted JenkinsHash();
    for (@Tainted int length = in.read(bytes); length > 0 ; length = in.read(bytes)) {
