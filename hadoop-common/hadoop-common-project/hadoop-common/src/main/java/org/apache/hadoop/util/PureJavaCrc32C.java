...
public class PureJavaCrc32C implements @Tainted Checksum {

  /** the current CRC value, bit-flipped */
  private @Tainted int crc;

  /** Create a new PureJavaCrc32 object. */
  public @Tainted PureJavaCrc32C() {
    reset();
  }

  @Override
  public @Tainted long getValue(@Tainted PureJavaCrc32C this) {
    @Tainted
    long ret = crc;
  ...
  @Override
  public void reset(@Tainted PureJavaCrc32C this) {
    crc = 0xffffffff;
  }

  @Override
  public void update(@Tainted PureJavaCrc32C this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) {
    @Tainted
    int localCrc = crc;

    while(len > 7) {
      final @Tainted int c0 =(b[off+0] ^ localCrc) & 0xff;
      final @Tainted int c1 =(b[off+1] ^ (localCrc >>>= 8)) & 0xff;
      final @Tainted int c2 =(b[off+2] ^ (localCrc >>>= 8)) & 0xff;
      final @Tainted int c3 =(b[off+3] ^ (localCrc >>>= 8)) & 0xff;
      localCrc = (T[T8_7_start + c0] ^ T[T8_6_start + c1])
          ^ (T[T8_5_start + c2] ^ T[T8_4_start + c3]);

      final @Tainted int c4 = b[off+4] & 0xff;
      final @Tainted int c5 = b[off+5] & 0xff;
      final @Tainted int c6 = b[off+6] & 0xff;
      final @Tainted int c7 = b[off+7] & 0xff;
  ...
  @Override
  final public void update(@Tainted PureJavaCrc32C this, @Tainted int b) {
  ...
  private static final @Tainted int T8_0_start = 0*256;
  private static final @Tainted int T8_1_start = 1*256;
  private static final @Tainted int T8_2_start = 2*256;
  private static final @Tainted int T8_3_start = 3*256;
  private static final @Tainted int T8_4_start = 4*256;
  private static final @Tainted int T8_5_start = 5*256;
  private static final @Tainted int T8_6_start = 6*256;
  private static final @Tainted int T8_7_start = 7*256;

  private static final @Tainted int @Tainted [] T = new @Tainted int @Tainted [] {
    /* T8_0 */
