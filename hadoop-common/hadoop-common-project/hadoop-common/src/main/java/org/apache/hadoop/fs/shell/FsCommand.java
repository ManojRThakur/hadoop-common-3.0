...
abstract public class FsCommand extends @Tainted Command {
  /**
  ...
  protected @Tainted FsCommand() {}
  
  protected @Tainted FsCommand(@Tainted Configuration conf) {
    super(conf);
  }

  // historical abstract method in Command
  @Override
  public @Tainted String getCommandName(@Tainted FsCommand this) { 
    return getName(); 
  }
  ...
  // overridden below
  @Override
  protected void run(@Tainted FsCommand this, @Tainted Path path) throws IOException {
    throw new @Tainted RuntimeException("not supposed to get here");
  }
  
  /** @deprecated use {@link Command#run(String...argv)} */
  @Deprecated
  @Override
  public @Tainted int runAll(@Tainted FsCommand this) {
    return run(args);
  }
}
