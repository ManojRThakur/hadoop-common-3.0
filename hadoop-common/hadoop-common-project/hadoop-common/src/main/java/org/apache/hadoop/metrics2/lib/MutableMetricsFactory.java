...
public class MutableMetricsFactory {
  private static final @Tainted Log LOG = LogFactory.getLog(MutableMetricsFactory.class);

  @Tainted
  MutableMetric newForField(@Tainted MutableMetricsFactory this, @Tainted Field field, @Tainted Metric annotation,
                            @Tainted
    ...
    final @Tainted Class<@Tainted ? extends java.lang.@Tainted Object> cls = field.getType();
    ...
    if (cls == MutableRates.class) {
      return new @Tainted MutableRates(registry);
    }
    ...
    throw new @Tainted MetricsException("Unsupported metric field "+ field.getName() +
                               " of type "+ field.getType().getName());
  }

  @Tainted
  MutableMetric newForMethod(@Tainted MutableMetricsFactory this, @Tainted Object source, @Tainted Method method, @Tainted Metric annotation,
                             @Tainted
                             MetricsRegistry registry) {
    if (LOG.isDebugEnabled()) {
    ...
    metric = metric != null ? metric :
        new @Tainted MethodMetric(source, method, info, annotation.type());
    registry.add(info.name(), metric);
    return metric;
  }
   ...
   */
  protected @Tainted MutableMetric newForField(@Tainted MutableMetricsFactory this, @Tainted Field field, @Tainted Metric annotation) {
    return null;
  }
   ...
   * @return a new metric object or null
   */
  protected @Tainted MutableMetric newForMethod(@Tainted MutableMetricsFactory this, @Tainted Object source, @Tainted Method method,
                                       @Tainted
                                       Metric annotation) {
    return null;
  }

  protected @Tainted MetricsInfo getInfo(@Tainted MutableMetricsFactory this, @Tainted Metric annotation, @Tainted Field field) {
    return getInfo(annotation, getName(field));
  }

  protected @Tainted String getName(@Tainted MutableMetricsFactory this, @Tainted Field field) {
    return StringUtils.capitalize(field.getName());
  }

  protected @Tainted MetricsInfo getInfo(@Tainted MutableMetricsFactory this, @Tainted Metric annotation, @Tainted Method method) {
    return getInfo(annotation, getName(method));
  }

  protected @Tainted MetricsInfo getInfo(@Tainted MutableMetricsFactory this, @Tainted Class<@Tainted ? extends java.lang.@Tainted Object> cls, @Tainted Metrics annotation) {
    @Tainted
   ...
   */
  protected @Tainted String getName(@Tainted MutableMetricsFactory this, @Tainted Method method) {
    @Tainted
  ...
  protected @Tainted MetricsInfo getInfo(@Tainted MutableMetricsFactory this, @Tainted Metric annotation, @Tainted String defaultName) {
    @Tainted
    String @Tainted [] value = annotation.value();
    if (value.length == 2) {
