...
public class HadoopKerberosName extends @Tainted KerberosName {

  /**
   * Create a name from the full Kerberos principal name.
   * @param name
   */
  public @Tainted HadoopKerberosName(@Tainted String name) {
    super(name);
  }
  /**
   ...
   * @throws IOException
   */
  public static void setConfiguration(@Tainted Configuration conf) throws IOException {
    final @Tainted String defaultRule;
    switch (SecurityUtil.getAuthenticationMethod(conf)) {
      case KERBEROS:
      case KERBEROS_SSL:
        try {
          KerberosUtil.getDefaultRealm();
        } catch (@Tainted Exception ke) {
          throw new @Tainted IllegalArgumentException("Can't get Kerberos realm", ke);
        }
        defaultRule = "DEFAULT";
        break;
      default:
  ...
  public static void main(@Tainted String @Tainted [] args) throws Exception {
    setConfiguration(new @Tainted Configuration());
    for(@Tainted String arg: args) {
      @Tainted
      HadoopKerberosName name = new @Tainted HadoopKerberosName(arg);
