...
public class XMLUtils {
  ...
  public static void transform(
                               @Tainted
                               InputStream styleSheet, @Tainted InputStream xml, @Tainted Writer out
                               ) 
    ...
    Transformer transformer = tFactory.newTransformer(
                                                      new @Tainted StreamSource(styleSheet)
                                                      );

    // Use the Transformer to transform an XML Source 
    // and send the output to a Result object.
    transformer.transform(new @Tainted StreamSource(xml), new @Tainted StreamResult(out));
  }
}
