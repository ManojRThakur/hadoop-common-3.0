...
public abstract class VersionUtil {
  ...
  private static final @Tainted Pattern COMPONENT_GROUPS = Pattern.compile("(\\d+)|(\\D+)");
   ...
   */
  private static final @Tainted String SNAPSHOT_SUFFIX = "-SNAPSHOT";
   ...
   *         versions are equal.
   */
  public static @Tainted int compareVersions(@Tainted String version1, @Tainted String version2) {
    @Tainted
    ...
    @Tainted
    String @Tainted [] version1Parts = version1.split("\\.");
    @Tainted
    String @Tainted [] version2Parts = version2.split("\\.");
    
    for (@Tainted int i = 0; i < version1Parts.length && i < version2Parts.length; i++) {
      @Tainted
  ...
  private static @Tainted String stripSnapshotSuffix(@Tainted String version) {
  ...
  private static @Tainted boolean isNumeric(@Tainted String s) {
    try {
      Integer.parseInt(s);
      return true;
