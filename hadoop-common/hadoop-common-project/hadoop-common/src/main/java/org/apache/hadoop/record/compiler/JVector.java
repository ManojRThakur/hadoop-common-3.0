...
public class JVector extends @Tainted JCompType {
  ...
  private @Tainted JType type;
  
  class JavaVector extends @Tainted JavaCompType {
    
    private @Tainted JType.@Tainted JavaType element;
    
    @Tainted
    JavaVector(@Tainted JType.@Tainted JavaType t) {
    ...
    @Override
    @Tainted
    String getTypeIDObjectString(@Tainted JVector.JavaVector this) {
      return "new org.apache.hadoop.record.meta.VectorTypeID(" + 
      element.getTypeIDObjectString() + ")";
    }

    @Override
    void genSetRTIFilter(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb, @Tainted Map<@Tainted String, @Tainted Integer> nestedStructMap) {
      element.genSetRTIFilter(cb, nestedStructMap);
    }

    @Override
    void genCompareTo(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb, @Tainted String fname, @Tainted String other) {
      cb.append("{\n");
      incrLevel();
    ...
    @Override
    void genReadMethod(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb, @Tainted String fname, @Tainted String tag, @Tainted boolean decl) {
      if (decl) {
    ...
    @Override
    void genWriteMethod(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb, @Tainted String fname, @Tainted String tag) {
      cb.append("{\n");
      incrLevel();
    ...
    @Override
    void genSlurpBytes(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb, @Tainted String b, @Tainted String s, @Tainted String l) {
      cb.append("{\n");
      incrLevel();
    ...
    @Override
    void genCompareBytes(@Tainted JVector.JavaVector this, @Tainted CodeBuffer cb) {
      cb.append("{\n");
      incrLevel();
  ...
  class CppVector extends @Tainted CppCompType {
    
    private @Tainted JType.@Tainted CppType element;
    
    @Tainted
    CppVector(@Tainted JType.@Tainted CppType t) {
    ...
    @Override
    @Tainted
    String getTypeIDObjectString(@Tainted JVector.CppVector this) {
      return "new ::hadoop::VectorTypeID(" +    
      element.getTypeIDObjectString() + ")";
    }

    @Override
    void genSetRTIFilter(@Tainted JVector.CppVector this, @Tainted CodeBuffer cb) {
  ...
  public @Tainted JVector(@Tainted JType t) {
    type = t;
    setJavaType(new @Tainted JavaVector(t.getJavaType()));
    setCppType(new @Tainted CppVector(t.getCppType()));
    setCType(new @Tainted CCompType());
  }
  
  @Override
  @Tainted
  String getSignature(@Tainted JVector this) {
