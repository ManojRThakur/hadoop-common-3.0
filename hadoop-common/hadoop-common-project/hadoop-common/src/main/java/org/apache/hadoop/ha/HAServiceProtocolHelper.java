...
public class HAServiceProtocolHelper {
  public static void monitorHealth(@Tainted HAServiceProtocol svc,
      @Tainted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
      svc.monitorHealth();
    } catch (@Tainted RemoteException e) {
      throw e.unwrapRemoteException(HealthCheckFailedException.class);
    }
  }

  public static void transitionToActive(@Tainted HAServiceProtocol svc,
      @Tainted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
  ...
  public static void transitionToStandby(@Tainted HAServiceProtocol svc,
      @Tainted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
