...
public class BlockCompressorStream extends @Tainted CompressorStream {

  // The 'maximum' size of input data to be compressed, to account
  // for the overhead of the compression algorithm.
  private final @Tainted int MAX_INPUT_SIZE;
   ...
   */
  public @Tainted BlockCompressorStream(@Tainted OutputStream out, @Tainted Compressor compressor, 
                               @Tainted
                               int bufferSize, @Tainted int compressionOverhead) {
   ...
   */
  @Override
  public void write(@Tainted BlockCompressorStream this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) throws IOException {
    // Sanity checks
    ...
    if (!compressor.needsInput()) {
      ...
      rawWriteInt((@Tainted int)compressor.getBytesRead());
      do {
        compress();
  ...
  @Override
  public void finish(@Tainted BlockCompressorStream this) throws IOException {
    if (!compressor.finished()) {
      rawWriteInt((@Tainted int)compressor.getBytesRead());
      compressor.finish();
      while (!compressor.finished()) {
  ...
  @Override
  protected void compress(@Tainted BlockCompressorStream this) throws IOException {
    @Tainted
  ...
  private void rawWriteInt(@Tainted BlockCompressorStream this, @Tainted int v) throws IOException {
    out.write((v >>> 24) & 0xFF);
    out.write((v >>> 16) & 0xFF);
    out.write((v >>>  8) & 0xFF);
    out.write((v >>>  0) & 0xFF);
