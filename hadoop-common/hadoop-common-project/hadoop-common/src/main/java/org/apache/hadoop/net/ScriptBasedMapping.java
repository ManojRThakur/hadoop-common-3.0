...
public final class ScriptBasedMapping extends @Tainted CachedDNSToSwitchMapping {

  /**
   * Minimum number of arguments: {@value}
   */
  static final @Tainted int MIN_ALLOWABLE_ARGS = 1;

  /**
   * Default number of arguments: {@value}
   */
  static final @Tainted int DEFAULT_ARG_COUNT = 
                     CommonConfigurationKeys.NET_TOPOLOGY_SCRIPT_NUMBER_ARGS_DEFAULT;

  /**
   * key to the script filename {@value}
   */
  static final @Tainted String SCRIPT_FILENAME_KEY = 
                     CommonConfigurationKeys.NET_TOPOLOGY_SCRIPT_FILE_NAME_KEY ;
  /**
   * key to the argument count that the script supports
   * {@value}
   */
  static final @Tainted String SCRIPT_ARG_COUNT_KEY =
                     CommonConfigurationKeys.NET_TOPOLOGY_SCRIPT_NUMBER_ARGS_KEY ;
  /**
   * Text used in the {@link #toString()} method if there is no string
   * {@value}
   */
  public static final @Tainted String NO_SCRIPT = "no script";
   ...
   *
   */
  public @Tainted ScriptBasedMapping() {
    super(new @Tainted RawScriptBasedMapping());
  }
   ...
   */
  public @Tainted ScriptBasedMapping(@Tainted Configuration conf) {
    this();
    setConf(conf);
  }
   ...
   */
  private @Tainted RawScriptBasedMapping getRawMapping(@Tainted ScriptBasedMapping this) {
    return (@Tainted RawScriptBasedMapping)rawMapping;
  }

  @Override
  public @Tainted Configuration getConf(@Tainted ScriptBasedMapping this) {
    return getRawMapping().getConf();
  }

  @Override
  public @Tainted String toString(@Tainted ScriptBasedMapping this) {
   ...
   */
  @Override
  public void setConf(@Tainted ScriptBasedMapping this, @Tainted Configuration conf) {
    super.setConf(conf);
  ...
  private static final class RawScriptBasedMapping
      extends @Tainted AbstractDNSToSwitchMapping {
    private @Tainted String scriptName;
    private @Tainted int maxArgs; //max hostnames per call of the script
     ...
     */
    @Override
    public void setConf (ScriptBasedMapping.@Tainted RawScriptBasedMapping this, @Tainted Configuration conf) {
      super.setConf(conf);
      if (conf != null) {
     ...
     */
    public @Tainted RawScriptBasedMapping() {}

    @Override
    public @Tainted List<@Tainted String> resolve(ScriptBasedMapping.@Tainted RawScriptBasedMapping this, @Tainted List<@Tainted String> names) {
      @Tainted
      List<@Tainted String> m = new @Tainted ArrayList<@Tainted String>(names.size());
      ...
      if (output != null) {
        @Tainted
        StringTokenizer allSwitchInfo = new @Tainted StringTokenizer(output);
     ...
     */
    private @Tainted String runResolveCommand(ScriptBasedMapping.@Tainted RawScriptBasedMapping this, @Tainted List<@Tainted String> args) {
      @Tainted
      int loopCount = 0;
      if (args.size() == 0) {
        return null;
      ...
      @Tainted
      StringBuilder allOutput = new @Tainted StringBuilder();
      @Tainted
      int numProcessed = 0;
      ...
      while (numProcessed != args.size()) {
        ...
        @Tainted
        List<@Tainted String> cmdList = new @Tainted ArrayList<@Tainted String>();
        ...
        @Untainted String userDir = (@Untainted String) System.getProperty("user.dir");
        if (userDir != null) {
          dir = new @Untainted File(userDir);
        }
        @Tainted
        ShellCommandExecutor s = new @Tainted ShellCommandExecutor(
            cmdList.toArray(new @Untainted String @Tainted [cmdList.size()]), dir);
        try {
          s.execute();
     ...
     */
    @Override
    public @Tainted boolean isSingleSwitch(ScriptBasedMapping.@Tainted RawScriptBasedMapping this) {
      return scriptName == null;
    }

    @Override
    public @Tainted String toString(ScriptBasedMapping.@Tainted RawScriptBasedMapping this) {
      return scriptName != null ? ("script " + scriptName) : NO_SCRIPT;
    }

    @Override
    public void reloadCachedMappings(ScriptBasedMapping.@Tainted RawScriptBasedMapping this) {
      // Nothing to do here, since RawScriptBasedMapping has no cache, and
      // does not inherit from CachedDNSToSwitchMapping
    }

    @Override
    public void reloadCachedMappings(ScriptBasedMapping.@Tainted RawScriptBasedMapping this, @Tainted List<@Tainted String> names) {
      // Nothing to do here, since RawScriptBasedMapping has no cache, and
      // does not inherit from CachedDNSToSwitchMapping
    }
  }
