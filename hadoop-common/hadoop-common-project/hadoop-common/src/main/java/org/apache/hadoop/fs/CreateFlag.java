...
@Tainted  CREATE((@Tainted short) 0x01),

  /**
   * Truncate/overwrite a file. Same as POSIX O_TRUNC. See javadoc for description.
   */

@Tainted  OVERWRITE((@Tainted short) 0x02),

  /**
   * Append to a file. See javadoc for more description.
   */

@Tainted  APPEND((@Tainted short) 0x04),

  /**
   * Force closed blocks to disk. Similar to POSIX O_SYNC. See javadoc for description.
   */

@Tainted  SYNC_BLOCK((@Tainted short) 0x08);

  private final @Tainted short mode;

  private @Tainted CreateFlag(@Tainted short mode) {
    this.mode = mode;
  }

  @Tainted
  short getMode(@Tainted CreateFlag this) {
    return mode;
  }
   ...
   */
  public static void validate(@Tainted EnumSet<@Tainted CreateFlag> flag) {
    if (flag == null || flag.isEmpty()) {
      throw new @Tainted HadoopIllegalArgumentException(flag
          + " does not specify any options");
    }
    final @Tainted boolean append = flag.contains(APPEND);
    final @Tainted boolean overwrite = flag.contains(OVERWRITE);
   ...
   * @throws HadoopIllegalArgumentException if the CreateFlag is invalid
   */
  public static void validate(@Tainted Object path, @Tainted boolean pathExists,
      @Tainted
      EnumSet<@Tainted CreateFlag> flag) throws IOException {
    validate(flag);
    final @Tainted boolean append = flag.contains(APPEND);
    final @Tainted boolean overwrite = flag.contains(OVERWRITE);
    if (pathExists) {
      if (!(append || overwrite)) {
        throw new @Tainted FileAlreadyExistsException("File already exists: "
            + path.toString()
            + ". Append or overwrite option must be specified in " + flag);
      }
    } else if (!flag.contains(CREATE)) {
      throw new @Tainted FileNotFoundException("Non existing file: " + path.toString()
