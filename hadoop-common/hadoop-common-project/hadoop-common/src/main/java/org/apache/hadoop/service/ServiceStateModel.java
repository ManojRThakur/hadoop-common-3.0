...
public class ServiceStateModel {
   ...
   */
  private static final @Tainted boolean @Tainted [] @Tainted [] statemap =
    new boolean[] @Tainted [] {
   ...
   */
  private volatile Service.@Tainted STATE state;
   ...
   * state.
   */
  public @Tainted ServiceStateModel(@Tainted String name) {
   ...
   */
  public @Tainted ServiceStateModel(@Tainted String name, Service. @Tainted STATE state) {
    this.state = state;
    this.name = name;
  }
   ...
   * @return the state
   */
  public Service.@Tainted STATE getState(@Tainted ServiceStateModel this) {
    return state;
  }
   ...
   * @return the state
   */
  public @Tainted boolean isInState(@Tainted ServiceStateModel this, Service. @Tainted STATE proposed) {
   ...
   * the desired state
   */
  public void ensureCurrentState(@Tainted ServiceStateModel this, Service. @Tainted STATE expectedState) {
    if (state != expectedState) {
      throw new @Tainted ServiceStateException(name+ ": for this operation, the " +
   ...
   */
  public synchronized Service.@Tainted STATE enterState(@Tainted ServiceStateModel this, Service.@Tainted STATE proposed) {
    checkStateTransition(name, state, proposed);
    Service.@Tainted STATE oldState = state;
   ...
   * @param proposed proposed new state
   */
  public static void checkStateTransition(@Tainted String name,
                                          Service.@Tainted STATE state,
                                          Service.@Tainted STATE proposed) {
    if (!isValidStateTransition(state, proposed)) {
      throw new @Tainted ServiceStateException(name + " cannot enter state "
   ...
   */
  public static @Tainted boolean isValidStateTransition(Service.@Tainted STATE current,
                                               Service.@Tainted STATE proposed) {
    @Tainted
    boolean @Tainted [] row = statemap[current.getValue()];
   ...
   */
  @Override
  public @Tainted String toString(@Tainted ServiceStateModel this) {
