 ...
 */
@DoNotPool
public class BuiltInGzipDecompressor implements @Tainted Decompressor {
  private static final @Tainted int GZIP_MAGIC_ID = 0x8b1f;  // if read as LE short int
  private static final @Tainted int GZIP_DEFLATE_METHOD = 8;
  private static final @Tainted int GZIP_FLAGBIT_HEADER_CRC  = 0x02;
  private static final @Tainted int GZIP_FLAGBIT_EXTRA_FIELD = 0x04;
  private static final @Tainted int GZIP_FLAGBIT_FILENAME    = 0x08;
  private static final @Tainted int GZIP_FLAGBIT_COMMENT     = 0x10;
  private static final @Tainted int GZIP_FLAGBITS_RESERVED   = 0xe0;

  // 'true' (nowrap) => Inflater will handle raw deflate stream only
  private @Tainted Inflater inflater = new @Tainted Inflater(true);

  private @Tainted byte @Tainted [] userBuf = null;
  private @Tainted int userBufOff = 0;
  private @Tainted int userBufLen = 0;

  private @Tainted byte @Tainted [] localBuf = new @Tainted byte @Tainted [256];
  private @Tainted int localBufOff = 0;

  private @Tainted int headerBytesRead = 0;
  private @Tainted int trailerBytesRead = 0;
  private @Tainted int numExtraFieldBytesRemaining = -1;
  private @Tainted PureJavaCrc32 crc = new @Tainted PureJavaCrc32();
  private @Tainted boolean hasExtraField = false;
  private @Tainted boolean hasFilename = false;
  private @Tainted boolean hasComment = false;
  private @Tainted boolean hasHeaderCRC = false;

  private @Tainted GzipStateLabel state;
...
@Tainted  FINISHED;
   ...
   */
  public @Tainted BuiltInGzipDecompressor() {
  ...
  @Override
  public synchronized @Tainted boolean needsInput(@Tainted BuiltInGzipDecompressor this) {
   ...
   */
  @Override
  public synchronized void setInput(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) {
    if (b == null) {
   ...
   */
  @Override
  public synchronized @Tainted int decompress(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len)
  throws IOException {
    @Tainted
    int numAvailBytes = 0;
    ...
    if (state == GzipStateLabel.DEFLATE_STREAM) {
      ...
      } catch (@Tainted DataFormatException dfe) {
        throw new @Tainted IOException(dfe.getMessage());
      }
   ...
   */
  private void executeHeaderState(@Tainted BuiltInGzipDecompressor this) throws IOException {
      ...
      if (hasHeaderCRC) {
        ...
        if (localBufOff >= 2) {
          ...
          if (headerCRC != (crc.getValue() & 0xffff)) {
            throw new @Tainted IOException("gzip header CRC failure");
          }
          localBufOff = 0;
          crc.reset();
   ...
   */
  private void executeTrailerState(@Tainted BuiltInGzipDecompressor this) throws IOException {
    ...
    if (state == GzipStateLabel.TRAILER_CRC) {
      ...
      if (localBufOff >= 4) {
        ...
        if (streamCRC != crc.getValue()) {
          throw new @Tainted IOException("gzip stream CRC failure");
        }
        localBufOff = 0;
        crc.reset();
   ...
   */
  public synchronized @Tainted long getBytesRead(@Tainted BuiltInGzipDecompressor this) {
   ...
   */
  @Override
  public synchronized @Tainted int getRemaining(@Tainted BuiltInGzipDecompressor this) {
    return userBufLen;
  }

  @Override
  public synchronized @Tainted boolean needsDictionary(@Tainted BuiltInGzipDecompressor this) {
    return inflater.needsDictionary();
  }

  @Override
  public synchronized void setDictionary(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) {
   ...
   */
  @Override
  public synchronized @Tainted boolean finished(@Tainted BuiltInGzipDecompressor this) {
    return (state == GzipStateLabel.FINISHED);
  }
   ...
   */
  @Override
  public synchronized void reset(@Tainted BuiltInGzipDecompressor this) {
  ...
  @Override
  public synchronized void end(@Tainted BuiltInGzipDecompressor this) {
    inflater.end();
  }
   ...
   *  [bit 5   encrypted]
   */
  private void processBasicHeader(@Tainted BuiltInGzipDecompressor this) throws IOException {
    if (readUShortLE(localBuf, 0) != GZIP_MAGIC_ID) {
      throw new @Tainted IOException("not a gzip file");
    }
    if (readUByte(localBuf, 2) != GZIP_DEFLATE_METHOD) {
      throw new @Tainted IOException("gzip data not compressed with deflate method");
    }
    @Tainted
    int flg = readUByte(localBuf, 3);
    if ((flg & GZIP_FLAGBITS_RESERVED) != 0) {
      throw new @Tainted IOException("unknown gzip format (reserved flagbits set)");
    }
  ...
  private void checkAndCopyBytesToLocal(@Tainted BuiltInGzipDecompressor this, @Tainted int len) {
  ...
  private void checkAndSkipBytes(@Tainted BuiltInGzipDecompressor this, @Tainted int len) {
    crc.update(userBuf, userBufOff, len);
    userBufOff += len;
    userBufLen -= len;
    headerBytesRead += len;
  ...
  private @Tainted boolean checkAndSkipBytesUntilNull(@Tainted BuiltInGzipDecompressor this) {
    @Tainted
  ...
  private void copyBytesToLocal(@Tainted BuiltInGzipDecompressor this, @Tainted int len) {
    System.arraycopy(userBuf, userBufOff, localBuf, localBufOff, len);
    localBufOff += len;
    userBufOff += len;
    userBufLen -= len;
  ...
  private @Tainted int readUByte(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off) {
    return ((@Tainted int)b[off] & 0xff);
  }

  // caller is responsible for not overrunning buffer
  private @Tainted int readUShortLE(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off) {
    return ((((b[off+1] & 0xff) << 8) |
             ((b[off]   & 0xff)     )) & 0xffff);
  }

  // caller is responsible for not overrunning buffer
  private @Tainted long readUIntLE(@Tainted BuiltInGzipDecompressor this, @Tainted byte @Tainted [] b, @Tainted int off) {
    return ((((@Tainted long)(b[off+3] & 0xff) << 24) |
             ((@Tainted long)(b[off+2] & 0xff) << 16) |
             ((@Tainted long)(b[off+1] & 0xff) <<  8) |
             ((@Tainted long)(b[off]   & 0xff)      )) & 0xffffffffL);
  }
