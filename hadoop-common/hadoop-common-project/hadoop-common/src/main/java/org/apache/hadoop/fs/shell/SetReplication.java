...
class SetReplication extends @Tainted FsCommand {
  public static void registerCommands(@Tainted CommandFactory factory) {
    factory.addClass(SetReplication.class, "-setrep");
  }
  
  public static final @Tainted String NAME = "setrep";
  public static final @Tainted String USAGE = "[-R] [-w] <rep> <path> ...";
  ...
  protected @Tainted short newRep = 0;
  protected @Tainted List<@Tainted PathData> waitList = new @Tainted LinkedList<@Tainted PathData>();
  protected @Tainted boolean waitOpt = false;
  
  @Override
  protected void processOptions(@Tainted SetReplication this, @Tainted LinkedList<@Tainted String> args) throws IOException {
    @Tainted
    CommandFormat cf = new @Tainted CommandFormat(2, Integer.MAX_VALUE, "R", "w");
    cf.parse(args);
    waitOpt = cf.getOpt("w");
    setRecursive(true);
    ...
    if (newRep < 1) {
      throw new @Tainted IllegalArgumentException("replication must be >= 1");
    }
  }

  @Override
  protected void processArguments(@Tainted SetReplication this, @Tainted LinkedList<@Tainted PathData> args)
  throws IOException {
  ...
  @Override
  protected void processPath(@Tainted SetReplication this, @Tainted PathData item) throws IOException {
    if (item.stat.isSymlink()) {
      throw new @Tainted PathIOException(item.toString(), "Symlinks unsupported");
    }
    
    if (item.stat.isFile()) {
      if (!item.fs.setReplication(item.path, newRep)) {
        throw new @Tainted IOException("Could not set replication for: " + item);
      }
   ...
   */
  private void waitForReplication(@Tainted SetReplication this) throws IOException {
    for (@Tainted PathData item : waitList) {
      ...
      while (!done) {
        ...
        out.print(".");
        out.flush();
        try {Thread.sleep(10000);} catch (@Tainted InterruptedException e) {}
      }
      out.println(" done");
    }
  }
