...
public abstract class FSLinkResolver<@Tainted T extends java.lang.@Tainted Object> {
   ...
   */
  public static @Tainted Path qualifySymlinkTarget(final @Tainted URI pathURI,
      @Tainted
      Path pathWithLink, @Tainted Path target) {
    // NB: makeQualified uses the target's scheme and authority, if
    // specified, and the scheme and authority of pathURI, if not.
    final @Tainted URI targetUri = target.toUri();
    final @Tainted String scheme = targetUri.getScheme();
    final @Tainted String auth = targetUri.getAuthority();
   ...
   */
  abstract public @Tainted T next(@Tainted FSLinkResolver<T> this, final @Tainted AbstractFileSystem fs, final @Tainted Path p)
   ...
   * @throws IOException
   */
  public @Tainted T resolve(@Tainted FSLinkResolver<T> this, final @Tainted FileContext fc, final @Tainted Path path) throws IOException {
    @Tainted
    int count = 0;
    T in = null;
    @Tainted
    ...
    for (@Tainted boolean isLink = true; isLink;) {
      try {
        in = next(fs, p);
        isLink = false;
      } catch (@Tainted UnresolvedLinkException e) {
        if (!fc.resolveSymlinks) {
          throw new @Tainted IOException("Path " + path + " contains a symlink"
        ...
        if (count++ > FsConstants.MAX_PATH_LINKS) {
          throw new @Tainted IOException("Possible cyclic loop while " +
