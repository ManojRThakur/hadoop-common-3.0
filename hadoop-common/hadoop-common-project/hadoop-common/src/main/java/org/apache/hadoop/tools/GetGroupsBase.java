 ...
 */
public abstract class GetGroupsBase extends @Tainted Configured implements @Tainted Tool {
  
  private @Tainted PrintStream out;
  
  /**
   * Create an instance of this tool using the given configuration.
   * @param conf
   */
  protected @Tainted GetGroupsBase(@Tainted Configuration conf) {
   ...
   */
  protected @Tainted GetGroupsBase(@Tainted Configuration conf, @Tainted PrintStream out) {
    super(conf);
    this.out = out;
  }
   ...
   */
  @Override
  public @Tainted int run(@Tainted GetGroupsBase this, @Tainted String @Tainted [] args) throws Exception {
    if (args.length == 0) {
      args = new @Tainted String @Tainted [] { UserGroupInformation.getCurrentUser().getUserName() }; 
    }

    for (@Tainted String username : args) {
      @Tainted
      StringBuilder sb = new @Tainted StringBuilder();
      sb.append(username + " :");
      for (@Tainted String group : getUgmProtocol().getGroupsForUser(username)) {
        sb.append(" ");
        sb.append(group);
      }
      out.println(sb);
   ...
   * @throws IOException
   */
  protected abstract @Tainted InetSocketAddress getProtocolAddress(@Tainted GetGroupsBase this, @Tainted Configuration conf)
      throws IOException;
   ...
   * @throws IOException
   */
  protected @Tainted GetUserMappingsProtocol getUgmProtocol(@Tainted GetGroupsBase this) throws IOException {
    @Tainted
