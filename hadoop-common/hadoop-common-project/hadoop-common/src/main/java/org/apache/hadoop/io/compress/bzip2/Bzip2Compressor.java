 ...
 * 
 */
public class Bzip2Compressor implements @Tainted Compressor {
  ...
  static final @Tainted int DEFAULT_BLOCK_SIZE = 9;
  static final @Tainted int DEFAULT_WORK_FACTOR = 30;

  private static final @Tainted Log LOG = LogFactory.getLog(Bzip2Compressor.class);

  // HACK - Use this as a global lock in the JNI layer.
  private static @Tainted Class<@Tainted Bzip2Compressor> clazz = Bzip2Compressor.class;

  private @Tainted long stream;
  private @Tainted int blockSize;
  private @Tainted int workFactor;
  private @Tainted int directBufferSize;
  private @Tainted byte @Tainted [] userBuf = null;
  private @Tainted int userBufOff = 0, userBufLen = 0;
  private @Tainted Buffer uncompressedDirectBuf = null;
  private @Tainted int uncompressedDirectBufOff = 0, uncompressedDirectBufLen = 0;
  private @Tainted boolean keepUncompressedBuf = false;
  private @Tainted Buffer compressedDirectBuf = null;
  private @Tainted boolean finish, finished;
   ...
   */
  public @Tainted Bzip2Compressor() {
   ...
   */
  public @Tainted Bzip2Compressor(@Tainted Configuration conf) {
   ...
   */
  public @Tainted Bzip2Compressor(@Tainted int blockSize, @Tainted int workFactor, 
                         @Tainted
   ...
   */
  @Override
  public synchronized void reinit(@Tainted Bzip2Compressor this, @Tainted Configuration conf) {
    reset();
    end(stream);
    if (conf == null) {
      stream = init(blockSize, workFactor);
  ...
  @Override
  public synchronized void setInput(@Tainted Bzip2Compressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) {
    if (b == null) {
  ...
  synchronized void setInputFromSavedData(@Tainted Bzip2Compressor this) {
    @Tainted
  ...
  @Override
  public synchronized void setDictionary(@Tainted Bzip2Compressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) {
    throw new @Tainted UnsupportedOperationException();
  }

  @Override
  public synchronized @Tainted boolean needsInput(@Tainted Bzip2Compressor this) {
  ...
  @Override
  public synchronized void finish(@Tainted Bzip2Compressor this) {
    finish = true;
  }
  
  @Override
  public synchronized @Tainted boolean finished(@Tainted Bzip2Compressor this) {
  ...
  @Override
  public synchronized @Tainted int compress(@Tainted Bzip2Compressor this, @Tainted byte @Tainted [] b, @Tainted int off, @Tainted int len) 
    throws IOException {
    if (b == null) {
      throw new @Tainted NullPointerException();
    }
   ...
   */
  @Override
  public synchronized @Tainted long getBytesWritten(@Tainted Bzip2Compressor this) {
    checkStream();
   ...
   */
  @Override
  public synchronized @Tainted long getBytesRead(@Tainted Bzip2Compressor this) {
    checkStream();
    return getBytesRead(stream);
  }

  @Override
  public synchronized void reset(@Tainted Bzip2Compressor this) {
    checkStream();
    end(stream);
    stream = init(blockSize, workFactor);
    finish = false;
  ...
  @Override
  public synchronized void end(@Tainted Bzip2Compressor this) {
    if (stream != 0) {
      end(stream);
      stream = 0;
    }
  ...
  private void checkStream(@Tainted Bzip2Compressor this) {
    if (stream == 0)
      throw new @Tainted NullPointerException();
  }
  
  private native static void initIDs(@Tainted String libname);
  private native static @Tainted long init(@Tainted int blockSize, @Tainted int workFactor);
  private native @Tainted int deflateBytesDirect(@Tainted Bzip2Compressor this);
