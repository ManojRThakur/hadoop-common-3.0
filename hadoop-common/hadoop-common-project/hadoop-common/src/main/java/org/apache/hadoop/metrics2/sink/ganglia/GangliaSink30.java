 ...
 * 
 */
public class GangliaSink30 extends @Tainted AbstractGangliaSink {

  public final @Tainted Log LOG = LogFactory.getLog(this.getClass());

  private static final @Tainted String TAGS_FOR_PREFIX_PROPERTY_PREFIX = "tagsForPrefix.";
  
  private @Tainted MetricsCache metricsCache = new @Tainted MetricsCache();

  // a key with a NULL value means ALL
  private @Tainted Map<@Tainted String, @Tainted Set<@Tainted String>> useTagsMap = new @Tainted HashMap<@Tainted String, @Tainted Set<@Tainted String>>();

  @Override
  @SuppressWarnings("unchecked")
  public void init(@Tainted GangliaSink30 this, @Tainted SubsetConfiguration conf) {
    super.init(conf);

    conf.setListDelimiter(',');
    @Tainted
    Iterator<@Tainted String> it = (@Tainted Iterator<@Tainted String>) conf.getKeys();
    while (it.hasNext()) {
      @Tainted
      ...
      if (propertyName.startsWith(TAGS_FOR_PREFIX_PROPERTY_PREFIX)) {
        ...
        @Tainted
        String @Tainted [] tags = conf.getStringArray(propertyName);
        @Tainted
        boolean useAllTags = false;
        @Tainted
        Set<@Tainted String> set = null;
        if (tags.length > 0) {
          set = new @Tainted HashSet<@Tainted String>();
          for (@Tainted String tag : tags) {
            tag = tag.trim();
  ...
  public void appendPrefix(@Tainted GangliaSink30 this, @Tainted MetricsRecord record, @Tainted StringBuilder sb) {
    @Tainted
    String contextName = record.context();
    @Tainted
    Collection<@Tainted MetricsTag> tags = record.tags();
    if (useTagsMap.containsKey(contextName)) {
      @Tainted
      Set<@Tainted String> useTags = useTagsMap.get(contextName);
  ...
  @Override
  public void putMetrics(@Tainted GangliaSink30 this, @Tainted MetricsRecord record) {
    ...
    try {
      ...
      if (!isSupportSparseMetrics()) {
        ...
        if (cachedMetrics != null && cachedMetrics.metricsEntrySet() != null) {
          for (Map.@Tainted Entry<@Tainted String, @Tainted AbstractMetric> entry : cachedMetrics
              .metricsEntrySet()) {
            @Tainted
        ...
        @Tainted
        Collection<@Tainted AbstractMetric> metrics = (@Tainted Collection<@Tainted AbstractMetric>) record
            .metrics();
        if (metrics.size() > 0) {
      ...
      throw new @Tainted MetricsException("Failed to putMetrics", io);
    }
  }

  // Calculate the slope from properties and metric
  private @Tainted GangliaSlope calculateSlope(@Tainted GangliaSink30 this, @Tainted GangliaConf gConf,
      @Tainted
   ...
   * @throws IOException
   */
  protected void emitMetric(@Tainted GangliaSink30 this, @Tainted String groupName, @Tainted String name, @Tainted String type,
      @Tainted
      String value, @Tainted GangliaConf gConf, @Tainted GangliaSlope gSlope) throws IOException {
